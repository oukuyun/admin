...
<script>
    var search = makeDataTable(
        "#datatable",
        "{{route('Admin.OperateRecord.index',[],false)}}",
        "GET",
        function(d) {
        ...
        [
            ...
            { data: "auditable_type",    className: "text-center", render: (data, type, full, meta) => { return `[${full.event}]${data}`}},
            { data: "ip_address",    className: "text-center"},
            { data: "created_at",    className: "text-center", render: (data) => { return TimeFormat(data);}},
        ],
        "",
        {
            ordering:true,
